...
typedef CompleterCallback = void Function(
    Completer completer, int function, Uint8List data);

/// MODBUS client
/// http://www.modbus.org/docs/Modbus_Application_Protocol_V1_1b.pdf
class ModbusClientImpl extends ModbusClient {
  final Logger log = Logger('ModbusClientImpl');

  final ModbusConnector _connector;

  final Map<PendingKey, PendingCallback> _pendingMap = HashMap();
  final Map<PendingKey, Completer> _waitingMap = HashMap();
  final Queue<Request> _waitingQueue = DoubleLinkedQueue();
  ...
  void _onConnectorData(int function, Uint8List data) {
    log.finest(
        "RECV: fn: ${function.toRadixString(16).padLeft(2, '0')}h data: ${dumpHexToString(data)}");
  ...
  void _onConnectorError(error, stackTrace) {
    for (var element in _waitingMap.values) {
      element.completeError(error, stackTrace);
    }
    for (var element in _pendingMap.values) {
      element.completer.completeError(error, stackTrace);
    }
    ...
    errorStopper(error);
    throw ModbusConnectException("Connector Error: $error"); // NEEDED?!
  }

  void _onConnectorClose() {
    for (var element in _waitingMap.values) {
      // errorStopper("Connector was closed before operation was completed");
      DataBase.updateEventList([
        4,
        'Переподключение',
        "Connector was closed before operation was completed",
      ]);
      element.completeError(ModbusConnectException(
          "Connector was closed before operation was completed"));
    }
    for (var element in _pendingMap.values) {
      // errorStopper("Connector was closed before operation was completed");
      ...
      element.completer.completeError(ModbusConnectException(
          "Connector was closed before operation was completed"));
    }
  ...
  void _sendData(int function, Uint8List data) {
    log.finest(
        "SEND: fn: ${function.toRadixString(16).padLeft(2, '0')}h data: ${dumpHexToString(data)}");
