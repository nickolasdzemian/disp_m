...
class AsciiConverter {
  static final Logger log = Logger('AsciiConverter');

  static Uint8List fromAscii(List<int> tcpData) {
    String str = String.fromCharCodes(tcpData);
    log.finest('From ASCII: $str');
    if (str.length % 2 != 0) {
      throw const ModbusException("ASCII string is not even count");
    }
    if (!str.endsWith(String.fromCharCodes([0x0d, 0xa]))) {
      throw const ModbusException("Invalid ASCII received");
    }
    List<int> ret = [];
